...
class ActivitiesController extends AdminController {
    ...
    public function index($cate_id = null, $model_id = null, $position = null,$group_id=null){
        ...
        //2.开始分页
        $activities = M('Activities'); // 实例化User对象
        // 进行分页数据查询 注意page方法的参数的前面部分是当前的页数使用 $_GET[p]获取
        $count      = $activities->where($map)->count();// 查询满足要求的总记录数
       ...
       //配置页码显示样式
        ...
        $lists = $activities->where($map)->limit($Page->firstRow.','.$Page->listRows)->select();
        foreach ($lists as &$list){
            $document = D('Document')->where(['id'=>$list['activity']])->find();
            $list['title'] = $document['title'];
            $list['description'] = $document['description'];
            $list['deadline'] = $document['deadline'];
        }
    ...
    public  function status(){
        ...
        $status = I('status');//接受修改的状态
        $data['status'] = $status;
        $result = D('Activities')->where(['id'=>$id])->save($data);//执行更新
        if($result){
