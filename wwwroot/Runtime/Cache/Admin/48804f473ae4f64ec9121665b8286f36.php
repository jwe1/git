...
<div class="cf">
	<div class="cf">
		...
		<a class="btn ajax-post confirm" url="<?php echo U('setStatus','status=1');?>" target-form="ids">启 用</a>
		<a class="btn ajax-post confirm" url="<?php echo U('setStatus','status=0');?>" target-form="ids">禁 用</a>
		<a class="btn ajax-post confirm" url="<?php echo U('setStatus','status=-1');?>" target-form="ids">删 除</a>
	</div>
		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sale): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>￥<?php echo ($sale["price"]); ?> <?php echo $sale['danwei'] ? '元/月' :'万/月'?></td>
						<td><?php echo date('Y-m-d h:i',$sale['create_time'])?></td>
						<td><?php echo date('Y-m-d',$sale['end_time'])?></td>
						<td><?php echo $sale['type'] ? '出租' :'销售'?></td>
						<td>
							<a class="text-info" title="编辑" href="<?php echo U('edit?id='.$sale['id']);?>">编辑</a>
							<a title="删除" href="<?php echo U('del?id='.$sale['id']);?>">删除</a>
						</td>
		...
		$('.ajax-post:eq(0)').click(function(){/*启用*/
			...
			tr.each(function(){
                ...
                $.ajax({
                    type:'POST',
                    data:['id'+':'+id,'status:'+0],
                    url: '/admin.php?s=/Sale/status.html	',
                    async:true,
                    dataType:'json',
