...
<div class="cf">
	...
	<div class="data-table table-striped">
		<table>
			...
			<tbody>
				<?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($list["id"]); ?>">
						...
						<td>
							<a class="text-info" title="查看" href="<?php echo U('detail',['id'=>$list['id']]);?>">查看详情</a>
							<a title="删除" href="<?php echo U('del',['id'=>$list['id']]);?>">删除</a>
	...
	<script type="text/javascript">
       ...
       /* highlight_subnav('/admin.php/User/index.html');*/
		...
		$('.ajax-post:eq(0)').click(function(){/*启用*/
			...
			tr.each(function(){
                ...
                $.ajax({
                    type:'POST',
                    data:['id'+':'+id,'status:'+0],
                    url: '/admin.php?s=/Repair/status.html	',
                    async:true,
                    dataType:'json',
        ...
        $('.change_status').click(function(){
            ...
            $.post("<?php echo U('change_status');?>",{'id':id},function(data){
				if(data){
				    alert('处理成功');
